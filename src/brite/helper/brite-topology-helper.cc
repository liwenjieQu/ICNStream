  ...
  for (el = edgeList.begin (); el != edgeList.end (); el++)
    {
      ...
      switch ((*el)->GetConf ()->GetEdgeType ())
        ...
        case brite::EdgeConf::RT_EDGE:
          ...
          edgeInfo.delay = -1;//Modified by Wenjie
...
NodeContainer
BriteTopologyHelper::GetLeafNodes() const
{
	return LeafNodes;
}

NodeContainer
BriteTopologyHelper::GetIntmNodes() const
{
	return IntmNodes;
}

NodeContainer
BriteTopologyHelper::GetProducers() const
{
	return Producers;
}
...
BriteTopologyHelper::BuildBriteTopology ()
{
	...
	//not using MPI so each AS is on system number
	for (uint32_t i = 0; i < m_numAs; ++i)
	{
		m_systemForAs.push_back (0);
	}
...
BriteTopologyHelper::ConstructTopology ()
{
  ...
  for (BriteTopologyHelper::BriteEdgeInfoList::iterator it = m_briteEdgeInfoList.begin (); it != m_briteEdgeInfoList.end (); ++it)
    {
	  uint32_t maxPackets = 2147483647;
	  m_britePointToPointHelper.SetQueueAttribute ("MaxPackets", UintegerValue (maxPackets));
      // Set the link delay
      ...
      m_britePointToPointHelper.SetDeviceAttribute("Mtu", UintegerValue(1e6));
  ...
  {
	  if ((*it).type == "RT_LEAF ")
		  LeafNodes.Add(m_nodes.Get ((*it).nodeId));
	  else
	  {
		  if((*it).type == "RT_STUB " && Producers.GetN() < 2)// Topology Specific
		  {
			  Producers.Add(m_nodes.Get ((*it).nodeId));
		  }
		  else
			  IntmNodes.Add(m_nodes.Get ((*it).nodeId));
	  }
  }
}
