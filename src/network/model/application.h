...
public:
  ...
  virtual inline void
  ResetCounter(uint32_t n) { };

  virtual inline std::pair<uint32_t, uint32_t>
  DelayHistory(const std::string& BR, uint32_t hop) { return std::make_pair(0, 0); };

  virtual inline std::pair<uint32_t, double>
  GetCondition()
  {
