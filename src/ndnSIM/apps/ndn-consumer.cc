...
Consumer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::Consumer")
	...
	.AddAttribute ("StartMeasureTime", "The time frame since when the performance is measured",
				   StringValue ("6300s"),
				   MakeTimeAccessor (&Consumer::m_measureStart),
				   MakeTimeChecker ())

	.AddAttribute ("NoCachePartition", "",
					BooleanValue (true),
					MakeBooleanAccessor (&Consumer::m_noCachePartition),
					MakeBooleanChecker ())

	.AddAttribute("NumberofChunks",
				"The total number of chunks per video file used in the simulation",
				StringValue("15"),
				MakeUintegerAccessor(&Consumer::m_maxNumchunk),
...
Consumer::CheckRetxTimeout ()
{
  ...
  Time limit("2s");
  //rto is an important value. Need to check
  ...
  while (!m_seqTimeouts.empty ())
    {
      ...
      if (entry->time + limit <= now) // timeout expired?
        {
          uint32_t seqNo = entry->seq;
          m_seqTimeouts.get<i_timestamp> ().erase (entry);
          m_seqRetxCounts[seqNo]++;
          OnTimeout (seqNo);
        }
      else
...
Consumer::SendPacket ()
{
  ...
  NS_LOG_INFO("[ndn-consumer]: OnInterest! Node: "<<this->m_node->GetId());
...
Consumer::OnData (Ptr<const Data> data)
{
  ...
  int hopCount = data->GetTSI() + 1;

  SeqTimeoutsContainer::iterator entry = m_seqLastDelay.find (seq);

/*
  entry = m_seqFullDelay.find (seq);
  NS_LOG_DEBUG ("<[VideoComsumer] on NodeID:" <<GetNode()->GetId()<< "\tDATA for " <<
		  data->GetName().toUri() <<"\tSpending time:  "<<Simulator::Now () - entry->time);
  if (entry != m_seqFullDelay.end ())
    {
      m_firstInterestDataDelay (this, seq, Simulator::Now () - entry->time, m_seqRetxCounts[seq], hopCount);
    }
*/
  std::pair<FullSeqTimeoutsContainer::index<i_seq>::type::iterator,
  FullSeqTimeoutsContainer::index<i_seq>::type::iterator> fulldelayrange = m_seqFullDelay.equal_range(seq);

  if(Simulator::Now().Compare(m_measureStart) >= 0)
  {
	  if (entry != m_seqLastDelay.end ())//Find it in the container, the interest has been sent befor
	  {
	      m_lastRetransmittedInterestDataDelay (this, seq, Simulator::Now () - entry->time, hopCount);
	  }
/*
	  for(FullSeqTimeoutsContainer::index<i_seq>::type::iterator iter = fulldelayrange.first;
			  iter != fulldelayrange.second; iter++)
	  {
		  if(iter == fulldelayrange.first)
		  {
			  uint32_t rxcount = m_seqRetxCounts.find(seq) != m_seqRetxCounts.end() ? m_seqRetxCounts[seq] : 0;
			  m_firstInterestDataDelay (this, seq, Simulator::Now () - iter->time, rxcount, hopCount);
		  }
		  else
			  m_firstInterestDataDelay (this, seq, Simulator::Now () - iter->time, 0, hopCount);
	  }
*/
  }
  /*
  FullSeqTimeoutsContainer::index<i_seq>::type::iterator delptr = m_seqFullDelay.find(seq);
  while(delptr != m_seqFullDelay.end())
  {
	  m_seqFullDelay.erase(delptr);
	  delptr = m_seqFullDelay.find(seq);
  }
  */
