      ...
      if (abs(latitude) > 0.001 && abs(latitude) > 0.001)
      ...
      else
        {
          ...
          if(name.find("Server") == 0)
        	  m_Server.Add(node);
          else if(name.find("Intermediate") == 0)
        	  m_Intermediate.Add(node);
  ...
  BOOST_FOREACH (Link &link, m_linksList)
    {
      ...
      p2p.SetDeviceAttribute("Mtu", UintegerValue(1e6));
