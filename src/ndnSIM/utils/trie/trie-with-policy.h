...
public:
  ...
  trie_with_policy (size_t bucketSize = 1, size_t bucketIncrement = 1)
    : trie_ (name::Component (), bucketSize, bucketIncrement) /* Maybe the virtual root of the trie tree with empty payload */
    , policy_ (*this)
  {
  }

  inline std::pair< iterator, bool >
  insert (const FullKey &key, typename PayloadTraits::insert_type payload, uint32_t sizzze)
  {
    ...
    if (item.second) // real insert
      {
        bool ok = policy_.insert (s_iterator_to (item.first), sizzze);
        if (!ok)
          {
