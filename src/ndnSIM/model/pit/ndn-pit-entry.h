...
public:
  ...
  /*
   * Added by Wenjie Li
   * Track the reward by each Face
   */
  typedef std::map<uint32_t, double> reward_dict;
   ...
   */
  virtual in_iterator
  AddIncoming (Ptr<Face> face, double r = 0);
  ...
  /*
   * Added by Wenjie Li
   * Get the Timestamp when the first Interest Entry for the same Name is created
   *
   */
  const Time &
  GetTimestamp () const;
...
protected:
  ...
  /*
   * Added by Wenjie Li
   * For DASCache Heuristic Algorithm
   */
  Time 	m_entryCreatedTime;	///Record when this entry is created in the simulation;
  /*
   * Added by Wenjie Li
   * For Recording Reinforcement Learning Reward
   */
  reward_dict m_pitreward;
};
