...
protected:
  ...
  void
  DelayedSatisfyPendingInterest (Ptr<const Data> data,
		  	  	  	  	  	  	 Ptr<Interest> interest);
...
protected:
  ...
  /*
   * Added by Wenjie Li
   * Statistics processing for DASCache Heuristic Algorithm
   */
protected:
  void RecordHopDelay(Ptr<Face> inFace,
          	  Ptr<const Data> data,
			  Ptr<pit::Entry> pitEntry);	//record the delay with its direct neighbours

  void AddStatistics(Ptr<const Interest>);	//Add this Interest to VideoStatistics table
  //void UpdateDelay(Ptr<pit::Entry>);	//Update the delay of requests for a certain bit rate
...
protected:
  ...
  Ptr<Node> m_node;
  Ptr<PopularitySummary> m_stat_summary;	///The summary of statistics collected by routers
  Ptr<VideoStatistics> m_stats;				///The statistics of Interests
  Ptr<TranStatistics>		m_tran;			///The statistics of states in MDP
...
//  bool m_choice;
  ...
  /*
   * Added by Wenjie Li
   * In order to measure the link utilization
   * with RecordHopDelay(...) function
   */
  //TracedCallback<Ptr<const Data>, uint32_t, uint32_t, Time> m_linkData;
