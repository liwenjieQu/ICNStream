...
public:
  ...
  */
  ...
  inline void SetBRBoundary(uint64_t n)
  {
	  m_BRbd = n;
	  m_wire = 0;
  }

  inline uint64_t GetBRBoundary() const
  {
	  return m_BRbd;
  }
/*
  inline void SetWindow(uint8_t n)
  {
	  m_inWindow = n;
	  m_wire = 0;
  }
  inline uint8_t GetWindow() const
  {
	  return m_inWindow;
  }

  inline void SetProducerRank(uint8_t n)
  {
	  m_producerRank = n;
	  m_wire = 0;
  }
  inline uint8_t GetProducerRank() const
  {
	  return m_producerRank;
  }
*/
private:
  // NO_ASSIGN
...
private:
  ...
  int8_t  		m_nackType;      ///< @brief Negative Acknowledgement type
  int8_t 		m_TSI;				//Used in ProbCache (for comparison) Wenjie
  uint32_t 		ProbeCache_ACC;

  uint64_t		m_BRbd;			//Used in Cache Multisection scheme to indicate the boundary of Concentric Ring
//  uint8_t		m_inWindow;		//Request still in the range of history Window.
//  uint8_t		m_producerRank;	//The Concentric-Ring on Video Producer.
