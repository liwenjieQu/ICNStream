...
Interest::GetSerializedSize (void) const
{
  size_t size =
    ...
    (4/*nonce*/ + 1/*scope*/ + 1/*nack type*/ + 2/*timestamp*/ + 1 /*TSI*/ + 4 /*ACC*/ +
     8 /*BR Boundary*/ + //1 /* In Window(FESTIVE) */ + 1 /* Producer Rank */ +
...
Interest::Serialize (Buffer::Iterator start) const
{
  ...
  start.WriteU8 (m_interest->GetTSI());
  start.WriteU32 (m_interest->GetAcc());

  start.WriteU64(m_interest->GetBRBoundary());
  //start.WriteU8(m_interest->GetWindow());
  //start.WriteU8(m_interest->GetProducerRank());
...
Interest::Deserialize (Buffer::Iterator start)
{
  ...
  m_interest->SetTSI(i.ReadU8());
  m_interest->SetAcc(i.ReadU32());

  m_interest->SetBRBoundary(i.ReadU64());
  //m_interest->SetWindow(i.ReadU8());
  //m_interest->SetProducerRank(i.ReadU8());
...
{
  uint32_t size = 1 /*version*/+ 1 /*packet type*/+ 2 /*length*/+
    ((2 + 2) /*signature*/+ 1 /*TSI*/+ 1 /*TSB*/+ 2 /*Reward*/ + 4 /*ACC*/+
     NdnSim::SerializedSizeName (m_data->GetName ()) +
     (2 + 2 + 4 + 2 + 2 + (2 + 0)))+
	 m_data->GetHops() * 4 + 1 /*1 Byte for m_hops*/;
...
Data::Serialize (Buffer::Iterator start) const
{
  ...
  start.WriteU8(m_data->GetTSI());
  start.WriteU8(m_data->GetTSB());
  start.WriteU32(m_data->GetAcc());

  // name
  NdnSim::SerializeName (start, m_data->GetName ());

  start.WriteU16(m_data->GetReward());
  start.WriteU8(m_data->GetHops());
  // content
  // for now assume that contentdata length is zero
  start.WriteU16 (2 + 4 + 2 + m_data->GetHops() * 4 + 2 + (2 + 0));
  start.WriteU16 (4 + 2 + m_data->GetHops() * 4 + 2 + (2 + 0));
  start.WriteU32 (static_cast<uint32_t> (m_data->GetTimestamp ().ToInteger (Time::MS)));
  ...
  if(m_data->GetHops() > 0)
  {
	  start.WriteU32 (static_cast<uint32_t> (m_data->m_mostRecentDelay.ToInteger (Time::MS)));
	  for(int i = 0; i < static_cast<int>(m_data->GetHops())-1; i++)
		  start.WriteU32(m_data->GetDelay(i));
  }
...
Data::Deserialize (Buffer::Iterator start)
{
  ...
  if (signatureLength == 6)
    ...
    throw new DataException ();
  m_data->SetTSI(i.ReadU8());
  m_data->SetTSB(i.ReadU8());
  m_data->SetAcc(i.ReadU32());

  m_data->SetName (NdnSim::DeserializeName (i));
  m_data->SetReward(i.ReadU16());
  m_data->SetHops(i.ReadU8());

  if (i.ReadU16 () != (2 + 4 + 2 + m_data->GetHops() * 4 + 2 + (2 + 0))) // content length
    throw new DataException () ;

  if (i.ReadU16 () != (4 + 2 + m_data->GetHops() * 4 + 2 + (2 + 0))) // Length (content Info)
    throw new DataException ();

  m_data->SetTimestamp (MilliSeconds (i.ReadU32 ()));
  m_data->SetFreshness (Seconds (i.ReadU16 ()));

  for(uint32_t k = 0; k < m_data->GetHops(); k++)
	  m_data->SetDelay(k, i.ReadU32());
