...
public:
  ...
  //ApplicationContainer VideoInstall (NodeContainer c, const std::string brArray[], int size);
  ApplicationContainer VideoInstall (Ptr<Node> node, const std::string brArray[], int size);
        
private:
  /*
   * Install the NDNBitRate Class, which contains the bitrate information for adaptive streaming
   * Should implement the function in a inherited class. Just lazy
   */
  void SetBitrateInfo(Ptr<Node> node, const std::string brArray[], int size);
  /**
   * \internal
   * Install an ns3::NdnConsumer on the node configured with all the 
   * attributes set with SetAttribute.
