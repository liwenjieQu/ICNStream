...
GlobalRoutingHelper::AddOrigin (const std::string &prefix, Ptr<Node> node)
{
  ...
  Ptr<Name> n = Create<Name> (boost::lexical_cast<Name> (prefix));
  m_originNames.push_back(n);
  gr->AddLocalPrefix (n);
}
...
void GlobalRoutingHelper::InstallCachedRoutes()
{
	for (NodeList::Iterator node = NodeList::Begin (); node != NodeList::End (); node++)
	{
		Ptr<L3Protocol> ndn = (*node)->GetObject<L3Protocol>();
		NS_LOG_DEBUG("[FIB Install]: Checking NodeID:" << (*node)->GetId());
		Ptr<ContentStore> cs = ndn->GetObject<ContentStore>();

		//cs travel through the trie tree to get the name of the cached content
		cs->FillinCacheRun();
		std::vector<Name>::iterator iter = cs->inCacheRun.begin();
		for(; iter != cs->inCacheRun.end(); iter++)
		{
			NS_LOG_DEBUG("[FIB Install]: At NodeID:" << (*node)->GetId() << " Name:" << iter->toUri());
			InstallCachedRecersive(*iter, *node, 0);
		}
	}
}

void GlobalRoutingHelper::SetTrigger(std::string t)
{
	Time trigger(t);
	Simulator::Schedule(trigger, &GlobalRoutingHelper::InstallCachedRoutes, this);
}

void GlobalRoutingHelper::InstallCachedRecersive(const Name& name, Ptr<Node> nodeptr, int32_t metric)
{
	metric++;
	std::list<Ptr<Name> >::iterator origin = m_originNames.begin();
	for(; origin != m_originNames.end(); origin++)
	{
		if((*origin)->compare(name) < 0)
		{
			Ptr<fib::Entry> fibentry = nodeptr->GetObject<Fib>()->Find(**origin);
			if(fibentry != 0)
			{
				//Since we use GlobalRouting, each fib entry has only one face.
                ...
                NS_LOG_DEBUG (" prefix " << name.toUri() << " reachable via face " << *fibentry_face
                              << " with distance " << entry->GetRoutingCost()
                              << " with delay " << 0);

				nodeptr->GetObject<Fib>()->Add(name, fibentry_face, entry->GetRoutingCost());
				...
				if(fibentry_netface != 0)
				{
					...
					if(p2pdevice != 0)
					{
						...
						Ptr<L3Protocol> nextNode_ndn = nextNode_device->GetNode()->GetObject<L3Protocol>();

						if(nextNode_device->GetNode()->GetNApplications() == 0)//Not a producer or consumer node
						{
			                NS_LOG_DEBUG (" prefix " << name.toUri() << " reachable via face " << *(nextNode_ndn->GetFaceByNetDevice(nextNode_device))
			                              << " with distance " << metric
			                              << " with delay " << 0);
							nextNode_ndn->GetObject<Fib>()->Add(name, nextNode_ndn->GetFaceByNetDevice(nextNode_device), metric);
							InstallCachedRecersive(name, nextNode_device->GetNode(), metric);
						}
					}
				}
			}
