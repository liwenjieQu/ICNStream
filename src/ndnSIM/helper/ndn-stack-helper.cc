  ...
  , m_needSetDefaultRoutes (false)
{
  ...
  m_contentStoreFactory.SetTypeId ("ns3::ndn::cs::CE2::Lru");
  m_fibFactory.         SetTypeId ("ns3::ndn::fib::Default");
  m_pitFactory.         SetTypeId ("ns3::ndn::pit::Persistent");

  m_videostatFactory.	SetTypeId ("ns3::ndn::VideoStatistics");
  m_popsummaryFactory.	SetTypeId ("ns3::ndn::PopularitySummary");
  m_cachedecisionFactory.SetTypeId ("ns3::ndn::VideoCacheDecision");
							  ...
							  const std::string &attr5, const std::string &value5)
{
  ...
  if (attr5 != "")
      m_contentStoreFactory.Set (attr5, StringValue (value5));
}

void
StackHelper::SetContentStore(const std::string &contentStore,
                              const std::string &attr1, const AttributeValue &value)
{
	m_contentStoreFactory.SetTypeId (contentStore);

	m_contentStoreFactory.Set(attr1, value);
}
...
void
StackHelper::SetPopSummary (const std::string &PopClass,
                     const std::string &attr1, const std::string &value1,
                     const std::string &attr2, const std::string &value2,
                     const std::string &attr3, const std::string &value3,
                     const std::string &attr4, const std::string &value4)
{
	m_popsummaryFactory.SetTypeId (PopClass);
  if (attr1 != "")
	  m_popsummaryFactory.Set (attr1, StringValue (value1));
  if (attr2 != "")
	  m_popsummaryFactory.Set (attr2, StringValue (value2));
  if (attr3 != "")
	  m_popsummaryFactory.Set (attr3, StringValue (value3));
  if (attr4 != "")
	  m_popsummaryFactory.Set (attr4, StringValue (value4));
}

void
StackHelper::SetCacheDecision (const std::string &CacheClass,
                     const std::string &attr1, const std::string &value1,
                     const std::string &attr2, const std::string &value2,
                     const std::string &attr3, const std::string &value3,
                     const std::string &attr4, const std::string &value4)
{
	m_cachedecisionFactory.SetTypeId (CacheClass);
  if (attr1 != "")
	  m_cachedecisionFactory.Set (attr1, StringValue (value1));
  if (attr2 != "")
	  m_cachedecisionFactory.Set (attr2, StringValue (value2));
  if (attr3 != "")
	  m_cachedecisionFactory.Set (attr3, StringValue (value3));
  if (attr4 != "")
	  m_cachedecisionFactory.Set (attr4, StringValue (value4));
}
...
Ptr<FaceContainer>
StackHelper::Install (const NodeContainer &c)
{
...
Ptr<FaceContainer>
StackHelper::DASCacheInstall (Ptr<Node> node, std::vector<Name> content, const std::string brArray[], uint32_t size)
{
	  ...
	  Ptr<ContentStore> cs = m_contentStoreFactory.Create<ContentStore> ();
	  cs->SetContentInCache(content);
	  ndn->AggregateObject (cs);

	  // Aggregate Video BitRate on node
	  Ptr<NDNBitRate> brinfo = m_bitrate_factory.Create<NDNBitRate>();
	  for(uint32_t i = 0; i < size; i++)
	  {
		  brinfo->AddBitRate(brArray[i]);
	  }
	  ndn->AggregateObject (brinfo);
...
Ptr<FaceContainer>
StackHelper::Install (Ptr<Node> node, const std::string brArray[], uint32_t size, bool needstats)
{
  // NS_ASSERT_MSG (m_forwarding, "SetForwardingHelper() should be set prior calling Install() method");
  Ptr<FaceContainer> faces = Create<FaceContainer> ();
  ...
  // Create and aggregate content store
  Ptr<ContentStore> csptr = m_contentStoreFactory.Create<ContentStore> ();
  csptr->InitContentStore(brArray, size);
  csptr->ReportPartitionStatus();

  ndn->AggregateObject (csptr);
  ...
  std::size_t foundc1 = nodename.find("Intermediate");
  std::size_t foundc2 = nodename.find("Edge");
  if(needstats && (foundc2 != std::string::npos))
  {
	  node->AggregateObject (m_videostatFactory.Create<VideoStatistics>());
  }
...
void
StackHelper::SetRatio(Ptr<Node> node, double r[], const std::string brArray[], uint32_t size)
{
	Ptr<ContentStore> csptr = node->GetObject<ContentStore>();
	if(csptr != 0)
	{
	  csptr->SetSectionRatio(brArray, r, size);
	}
}

Ptr<FaceContainer>
StackHelper::StreamCacheInstall (Ptr<Node> node, bool isedge, const std::string brArray[], uint32_t size)
{
  // NS_ASSERT_MSG (m_forwarding, "SetForwardingHelper() should be set prior calling Install() method");
  Ptr<FaceContainer> faces = Create<FaceContainer> ();
  ...
  if(isedge)
	  ndn->AggregateObject (m_videostatFactory.Create<VideoStatistics>());

  ndn->AggregateObject (m_popsummaryFactory.Create<PopularitySummary>());

  ndn->AggregateObject (m_cachedecisionFactory.Create<VideoCacheDecision>());
...
Ptr<FaceContainer>
StackHelper::AdaptiveStreamInstall (Ptr<Node> node)
{
	Ptr<FaceContainer> faces = Create<FaceContainer> ();
	return faces;
}
...
Ptr<NetDeviceFace>
StackHelper::DefaultNetDeviceCallback (Ptr<Node> node, Ptr<L3Protocol> ndn, Ptr<NetDevice> netDevice)
{
...
Ptr<NetDeviceFace>
StackHelper::PointToPointNetDeviceCallback (Ptr<Node> node, Ptr<L3Protocol> ndn, Ptr<NetDevice> device)
{
...
Ptr<FaceContainer>
StackHelper::Install (const std::string &nodeName)
{
