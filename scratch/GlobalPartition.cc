...
string TotalChunk = "25";
string TotalFile = "60";
string ProbCache = "0.98";

string Zipf = "0.8";
double CachePercentage = 0.2;
double CacheRatio = 1;
...
NS_LOG_COMPONENT_DEFINE ("script.GlobalPartition");

int main (int argc, char *argv[])
{
//	LogComponentEnable("ndn.CachePartition", LOG_LEVEL_DEBUG);
//	LogComponentEnable("ndn.VideoClient", LOG_LEVEL_DEBUG);
...
//	LogComponentEnable("ndn.cs.Nocache", LOG_LEVEL_DEBUG);
	...
	Noncachedndnhelper.SetContentStore("ns3::ndn::cs::Nocache",
			...
			"RewardUnit", boost::lexical_cast<std::string>(rewardParam),
			"RewardDesign", boost::lexical_cast<std::string>(rewardDesign));
	Noncachedndnhelper.Install(ServerNode, m_bitratename, TotalBitrate);

	sql::Driver* driver = nullptr;
	driver = get_driver_instance();
	int dbport = (IP == "127.0.0.1") ? 9001 : 3306;
	std::string logpath;
	//logpath = "./Log/" + boost::lexical_cast<std::string>(ExpID) + ".txt";
	//std::cout << logpath << std::endl;
	ndn::PartitionHelper cachepartition (std::stoi(TotalFile), std::stoi(TotalChunk), TotalBitrate, TotalNode,
										m_bitratename, EdgeCacheSize, nonEdgeCacheSize, myprefix, roundtime, rewardParam, ExpID, rewardDesign, iterationTimes, logpath,
										driver, DB, MySQLUsername, MySqlPwd, IP, dbport);
	cachepartition.SetTopologicalOrder(ServerNode, EdgeNodes);
...
//	consumerHelper.SetAttribute("TargetBuffer", StringValue(targetbuff));
	...
	Simulator::Schedule(Time(triggertime), &ndn::PartitionHelper::IterativeRun, &cachepartition);
					 ...
					 NodeContainer& EdgeNodes, NodeContainer& IntmNodes)
{
	...
	{
		nosection_e.Install(*i, m_bitratename, TotalBitrate);
	}
	for(NodeContainer::Iterator i = IntmNodes.Begin(); i != IntmNodes.End(); i++)
	{
		nosection_i.Install(*i, m_bitratename, TotalBitrate);
	}
}
...
{//
	...
	{
		clientstackhelper.Install(*i, m_bitratename, TotalBitrate);
